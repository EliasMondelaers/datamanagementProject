...
1. Data exploration using visualization
2. Transformation of the target variable (using vote_average to establish a threshold to convert this continious target variable in to a binary target variable (recommend = 1 --> yes, recommend = 0 --> no)
3. Data cleaning + feature engineering
4. Preparation for modeling (train-test split, standardization, creation of interaction feature,..)
5. Modeling using simple models (decision trees, logistic regression, knn) and using their simplicity to interpret the decision making + model evaluation using accuracy
6. Modeling using black box models (XGBoost, Random Forest), and providing feature importance plots + model evaluation using accuracy
7. Creation of bonus regression model: to see if we could also directly predict the rating.
